...
- Highlights 
1. The adversary supplies a parameter such as URL to a sink that supports dynamic code execution, could enabling the hijacking of another account. DOM-based XSS can arise from the improper handling of HTML functions that send dynamic content, load scripts on a web page, or render malicious attributes, such as onload="alert()".
2. Exploting source to sinks. DOM based XSS vulnerability includes two elemenets, the source of user inputs and the target where the user input is written.
...
2. Content type to a non-standard value, which is anything that is not "application/x-www-form-urlencoded", "multipart/form-data", or "text/plain"
3. Pre-flight check was added to the CORS specification to protect legacy resources from the expanded request options allowed by CORS. 
4. Be test all different type of request to verfify which one have CORS vulnerability.
...
Kep points to looking for exploit of deserialization
- 1. Instances that can be leveraged within the application.
- 2. The value being accessed or manipulated.
- 3. Methods to wrap the object effectively.
