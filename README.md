...
SameSite restriction levels as below:
Strict - If a cookie is set with the SameSite=Strict attribute, browsers will not send it in any cross-site requests.
Lax - The browsers will send the cookie in cross-site request, but only in a few scenarios as the request uses the GET methond and the request resulted from a top-level navigation by the user.
